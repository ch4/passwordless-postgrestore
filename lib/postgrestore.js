 ...
 * (1) { pgstore: { difficulty: integer }} to change the bcrypt difficulty. Defaults to: 10
...
function PostgreStore(conString, options) {
  ...
  this._client.connect(function(err, client) {
    ...
    this._client = new pg.Pool({
        ...(this._ssl && Object.keys(this._ssl).length > 0 && this._ssl.constructor === Object) && {ssl: this._ssl},
        connectionString: conString,
        max: this._options.pgstore.pgPoolSize || 10,
    });
...
PostgreStore.prototype.storeOrUpdate = function(token, uid, msToLive, originUrl, callback) {
  ...
  var self = this;
  bcrypt.hash(token, self._difficulty, function(err, hashedToken) {
    if(err) {
      return callback(err);
    }
